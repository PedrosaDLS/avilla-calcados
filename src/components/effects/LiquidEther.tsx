...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      private hasInitializedCoords = false;
      ...
      init(container: HTMLElement, bounds: HTMLElement | null) {
        this.container = container;
        this.boundsElement = bounds ?? container;
      ...
      private getBoundsElement() {
        return this.boundsElement || this.container;
      }
      private isPointInside(clientX: number, clientY: number) {
        const el = this.getBoundsElement();
        if (!el) return false;
      ...
      update() {
        ...
        this.coords_old.copy(this.coords);
        if (!this.hasInitializedCoords) {
          this.hasInitializedCoords = true;
          this.diff.set(0, 0);
        }
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        Mouse.init(props.$wrapper, props.$bounds);
    ...
    const webgl = new WebGLManager({
      $wrapper: container,
      $bounds: boundsRef?.current ?? container,
      autoDemo,
      autoSpeed,
      autoIntensity,
      takeoverDuration,
